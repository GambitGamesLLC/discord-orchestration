#!/bin/bash
#
# worker-manager-discord.sh - Manage Discord-enabled workers
...
start_worker() {
    ...
    local TOKEN="${WORKER_TOKENS[$((WORKER_NUM-1))]:-}"
    ...
    while true; do
        ...
        export TASK_QUEUE_CHANNEL="${TASK_QUEUE_CHANNEL:-task-queue}"
        export RESULTS_CHANNEL="${RESULTS_CHANNEL:-results}"
        export WORKER_POOL_CHANNEL="${WORKER_POOL_CHANNEL:-worker-pool}"
        export POLL_INTERVAL="5"
        export MAX_IDLE_TIME="300"
        
        bash "${SCRIPT_DIR}/worker-discord.sh" > >(sed "s/^/[${WORKER_ID}] /") 2>&1 &
        local PID=$!
...
shutdown() {
    ...
    sleep 2
    pkill -f "worker-discord.sh" 2>/dev/null || true
